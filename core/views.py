...
class CheckoutView(View):
    def get(self, *args, **kwargs):
        try:
            ...
            shipping_address_qs = models.Address.objects.filter(user=self.request.user, address_type='S',default=True)
            if shipping_address_qs.exists():
                context.update({'default_shipping_address': shipping_address_qs[0]})

            billing_address_qs = models.Address.objects.filter(user=self.request.user, address_type='B',default=True)
            if shipping_address_qs.exists():
        ...
        try:
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    ...
                    address_qs = models.Address.objects.filter(user=self.request.user, address_type='S',default=True)
                    ...
                    if is_valid_form([shipping_address1,shipping_country,shipping_zip]):
                        ...
                        messages.warning(sef.request, "Please fill inthe required shipping address fields")
                ...
                if same_billing_address:
                    ...
                    address_qs = models.Address.objects.filter(user=self.request.user, address_type='B',default=True)
                    if address_qs.exists():
                    ...
                    if is_valid_form([billing_address1,billing_country,billing_zip]):
                        ...
                        messages.warning(sef.request, "Please fill inthe required billing address fields")
...
class PaymentView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            # messages.warning(self.request, f"{e.get('message')}")
            return redirect("/")
...
def add_to_cart(request, slug):
    ...
    order_item, created = models.OrderItem.objects.get_or_create(item=item, user=request.user,is_ordered=False)
...
def remove_from_cart(request,slug):
    ...
    if order_qs.exists():
        ...
        # check if the order item is in the order
        if order.items.filter(item__slug=item.slug).exists():
            order_item = models.OrderItem.objects.get_or_create(item=item, user=request.user,is_ordered=False)[0]
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            if order_item.quantity == 0:
                order.items.remove(order_item)
                messages.info(request, "The item " + order_item.item.title + " was removed from your cart.")
                return redirect('core:order-summary')
            else:
                order_item.save()
                messages.info(request, "The item quantity was updated.")
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                messages.info(self.request, "The coupon is now applied to the order.")
                return redirect("core:checkout")
